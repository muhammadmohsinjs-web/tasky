...
export function AddTaskModal({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isHabit && time.trim() && endTime.trim() && endTime.trim() <= time.trim()) {
    ...
    const recurrence: RecurrenceRule | null = isHabit
      ? {
          frequency: 'daily',
          interval: 1,
          end_date: null,
    ...
    try {
      didSave = await onSubmit({
        ...
        description: isHabit ? null : (description.trim() || null),
        notes: null,
        time: time.trim() || null,
        ...
        <header className="mb-5 border-b border-[#E8EDF5] pb-4">
          ...
          {!isViewMode ? (
            <p className="mt-1 text-sm text-[#64748B]">
              {taskType === 'habit'
                ? 'Keep habits lightweight. Add only what you need to stay consistent.'
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <section className="rounded-2xl border border-[#E4EBF6] bg-[#FBFCFF] p-4">
            ...
            <label className="block">
              ...
              <input
                ...
                placeholder={taskType === 'habit' ? 'Habit name' : 'Task title'}
              />
            ...
            {taskType === 'habit' ? (
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <label className="block md:col-span-2">
                  <span className="mb-1 block text-sm font-medium text-[#314866]">Start date (optional)</span>
                  <CalendarDateInput
                    value={dateISO}
                    onChange={setDateISO}
                    disabled={isViewMode}
                ...
                {scheduleError ? <p className="text-xs text-red-500 md:col-span-2">{scheduleError}</p> : null}
              </div>
            ) : (
              <>
          ...
          <section className="rounded-2xl border border-[#E4EBF6] bg-[#FBFCFF] p-4">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-[#233553]">Advanced <span className="text-[#7D8EA8]">(Optional)</span></h3>
              <button
                type="button"
                onClick={() => setShowAdvanced((prev) => !prev)}
                className="text-xs font-semibold text-[#2A63B2]"
              >
            ...
            {showAdvanced ? (
              <>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  ...
                  {taskType === 'habit' ? (
                    <label className="block">
                      <span className="mb-1 block text-sm font-medium text-[#314866]">End time (optional)</span>
                      <input
                        value={endTime}
                        onChange={event => {
                ...
                {taskType === 'task' ? (
                  <div className="mt-4">
            <h4 className="mb-3 text-sm font-semibold text-[#233553]">Resources</h4>
          ...
          <div className="flex justify-end gap-2 border-t border-[#E8EDF5] pt-4">
            ...
            {!isViewMode ? (
              <button
                ...
                {submitting ? 'Saving...' : isEditMode ? 'Save Changes' : taskType === 'habit' ? 'Create Habit' : 'Create Task'}
              </button>
            ) : null}
          </div>
        </form>
