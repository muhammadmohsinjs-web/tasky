...
export default function CalendarPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f2f4fa] px-3 py-4 md:px-6 md:py-6">
      <div className="mx-auto max-w-[1880px]">
        <section className="mb-4 rounded-[22px] border border-[#E4E8F0] bg-[#FCFDFF] px-6 py-7 shadow-[0_8px_24px_rgba(40,56,90,0.08)] md:px-10 md:py-10">
          <p className="text-[14px] font-semibold uppercase tracking-[0.12em] text-[#74809A]">Execution</p>
          <h1 className="mt-3 text-[46px] font-semibold leading-none tracking-[-0.02em] text-[#151C2E]">Tasks</h1>
          <p className="mt-6 text-[22px] text-[#5E6A80]">Manage your calendar, list view, and backlog in one workflow.</p>
        </section>

        <section className="mb-5 rounded-[22px] border border-[#E4E8F0] bg-[#FCFDFF] px-6 py-5 shadow-[0_8px_24px_rgba(40,56,90,0.08)] md:px-10">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="inline-flex h-[68px] items-center overflow-hidden rounded-[16px] border border-[#CBD4E3] bg-[#EFF2F7] shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]">
            ...
            <div className="min-w-[290px] flex-1 xl:max-w-[520px]">
              <div className="mb-2 flex items-center justify-between gap-3">
                <p className="text-[20px] font-semibold text-[#2A3650]">Tasks Progress</p>
                <label className="text-[14px] text-[#5D6880]">
                  Status
                  <select
                    value={statusFilter}
                    onChange={(event) => setStatusFilter(event.target.value as 'all' | TaskStatus)}
                    className="ml-2 h-8 rounded-md border border-[#CFD7E4] bg-white px-2 text-[13px] text-[#40506A]"
            ...
            <button
              ...
              className="inline-flex h-[66px] items-center justify-center gap-3 rounded-[16px] bg-gradient-to-r from-[#2A2DEB] to-[#0A6CE8] px-7 text-white shadow-[0_10px_24px_rgba(25,68,220,0.35)] transition hover:brightness-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
            >
              <Plus className="h-7 w-7" />
              <span className="text-[16px] font-semibold">Bulk Add</span>
              <span className="inline-flex h-9 items-center rounded-[10px] border border-white/20 bg-white/16 px-3 text-[15px] font-semibold">⌘ B</span>
            </button>
          </div>
        </section>

        <section className="mb-5 rounded-[20px] border border-[#E4E8F0] bg-[#FCFDFF] px-4 py-3 shadow-[0_8px_24px_rgba(40,56,90,0.08)] md:px-7">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-1">
              <ViewTab label="Calendar" value="calendar" activeView={activeView} onChange={setActiveView} icon={<CalendarDays className="h-5 w-5" />} />
              <ViewTab label="List" value="list" activeView={activeView} onChange={setActiveView} icon={<List className="h-5 w-5" />} />
              <ViewTab label="Backlog" value="backlog" activeView={activeView} onChange={setActiveView} icon={<Inbox className="h-5 w-5" />} />
            </div>

            <div className="flex items-center gap-3">
              <div className="inline-flex h-[66px] items-center overflow-hidden rounded-[16px] border border-[#C9D2E3] bg-[#F6F8FC]">
                <button
                  type="button"
                  onClick={() => handleMonthChange(-1)}
                  className="inline-flex h-full w-[62px] items-center justify-center border-r border-[#CFD7E4] text-[#364562] hover:bg-white"
                  aria-label="Previous month"
                >
                  <ChevronLeft className="h-7 w-7" />
                </button>
                <div className="inline-flex h-full min-w-[250px] items-center justify-center gap-3 px-5 text-[#2A3650]">
                  <CalendarIcon className="h-7 w-7 text-[#2A7CF2]" />
                  <span className="text-[17px] font-semibold">{formatMonthLabel(monthDate)}</span>
                </div>
                <button
                  type="button"
                  onClick={() => handleMonthChange(1)}
                  className="inline-flex h-full w-[62px] items-center justify-center border-l border-[#CFD7E4] text-[#364562] hover:bg-white"
              ...
              <button
                ...
                className="h-[66px] rounded-[14px] border border-[#CFD7E4] bg-white px-5 text-[15px] font-semibold text-[#3C4A65] hover:bg-[#F8FAFD]"
              >
                Today
              </button>
            </div>
        ...
        <div className="mb-4 rounded-[16px] border border-[#E0E6F0] bg-white px-4 py-3 shadow-[0_6px_16px_rgba(40,56,90,0.06)] md:px-6">
          <input
            type="search"
            value={searchQuery}
            ...
            className="h-11 w-full rounded-xl border border-[#D4DDEB] bg-[#F7F9FD] px-4 text-base text-[#344257] placeholder:text-[#7D879A] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          />
        </div>

        {activeView === 'calendar' ? (
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,3fr)_minmax(340px,1fr)]">
            <CalendarGrid
              monthDate={monthDate}
              ...
              <div className="space-y-5">
                {groupedMonthTasks.map((group) => (
                  <div key={group.dateKey}>
                    <p className="mb-2 text-sm font-medium text-slate-500">{parseISODate(group.dateKey).toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}</p>
...
function DomainTab({
  ...
  return (
    <button
      ...
      className={[
        'relative inline-flex h-full min-w-[125px] items-center justify-center gap-2 px-5 text-[16px] font-semibold transition',
        withDivider ? 'before:absolute before:left-0 before:top-1/2 before:h-9 before:w-px before:-translate-y-1/2 before:bg-[#C9D2E3]' : '',
        active
          ? 'rounded-[14px] border border-[#C9D2E3] bg-white text-[#222E46] shadow-[0_5px_15px_rgba(37,55,89,0.16)]'
          : 'text-[#36455F] hover:bg-white/60',
      ].join(' ')}
    >
      {label}
      {showDot ? <span className="h-2.5 w-2.5 rounded-full bg-[#F2C145]" aria-hidden="true" /> : null}
    </button>
  );
}
...
function ViewTab({
  ...
  return (
    <button
      ...
      className="group relative inline-flex h-[56px] min-w-[146px] items-center justify-center gap-2 px-4 text-[#2E3A52]"
      aria-pressed={active}
    >
      {icon}
      <span className="text-[16px] font-semibold">{label}</span>
      <span className={`absolute bottom-0 h-[5px] w-[88px] rounded-full bg-[#2A7CF2] transition ${active ? 'opacity-100' : 'opacity-0'}`} />
    </button>
  );
}
