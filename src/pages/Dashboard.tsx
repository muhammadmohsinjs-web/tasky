        ...
        <div className="panel lg:col-span-2 overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {recentTasks.length === 0 ? (
              <div className="px-5 py-8 text-center text-sm text-slate-400">No tasks yet. Create your first task.</div>
            ) : (
              recentTasks.map((task) => (
                <div
                  key={task.id}
                  onClick={() => navigate('/tasks')}
                  className="group px-5 py-3.5 flex items-center gap-3 hover:bg-slate-50 cursor-pointer"
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => {
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`text-sm truncate ${task.status === 'done' ? 'line-through text-slate-400' : 'text-slate-700'}`}>{task.title}</div>
